...
class CharacteristicOperationFragment: Fragment() {
    ...
    fun showData() {
        ...
        val characteristic = (activity as MainActivity).getCharacteristic()
        val charaProp = (activity as MainActivity).getCharaProp()
        ...
        if (childList.contains(child)) {
            ...
            txt_title.text =
                characteristic.uuid.toString() + activity!!.getString(R.string.data_changed)
            val txt = view.findViewById(R.id.txt) as TextView
            txt.movementMethod = ScrollingMovementMethod.getInstance()

            when (charaProp) {
                PROPERTY_READ -> {
                    val view_add = LayoutInflater.from(activity)
                        .inflate(R.layout.layout_characteric_operation_button, null)
                    val btn = view_add.findViewById(R.id.btnSigECG1) as Button
